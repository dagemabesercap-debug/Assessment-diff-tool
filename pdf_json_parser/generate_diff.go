...
func readJSONFile(path string) ([]Question, error) {
	f, err := os.Open(path)
	if err != nil {
		return nil, err
	}
...
func main() {
	q2025, err := readJSONFile("Serent Capital - Studio Designer (2025).pdf.json")
	if err != nil {
		fmt.Printf("Error reading 2025 JSON: %v\n", err)
		return
	}
	q2026, err := readJSONFile("Serent Capital - Studio Designer (2026).pdf.json")
	if err != nil {
		fmt.Printf("Error reading 2026 JSON: %v\n", err)
		return
	}
	...
	outputBytes, err := json.MarshalIndent(diffs, "", "  ")
	if err != nil {
		fmt.Printf("Error marshalling output JSON: %v\n", err)
		return
	}

	outPath := "../diff_report.json"
	err = os.WriteFile(outPath, outputBytes, 0644)
	if err != nil {
		fmt.Printf("Error writing diff report: %v\n", err)
		return
	}

	fmt.Printf("Saved %d aligned questions to %s\n", len(diffs), outPath)

	// Write JavaScript data file for CORS-free dashboard loading
	jsPath := "../diff_report_data.js"
	jsContent := []byte(fmt.Sprintf("const diffReportData = %s;\n", string(outputBytes)))
	err = os.WriteFile(jsPath, jsContent, 0644)
	if err != nil {
		fmt.Printf("Error writing diff report data JS: %v\n", err)
		return
	}
	fmt.Printf("Saved Javascript data file to %s\n", jsPath)
}
...
func alignNewQuestion(q26 Question) AlignedQuestionDiff {
	...
	// All 2026 evidence items are "added"
...
func alignDeletedQuestion(q25 Question) AlignedQuestionDiff {
	...
	// All 2025 evidence items are "removed"
...
func alignOptions(opts25, opts26 []Option) []OptionDiff {
	...
	for _, opt26 := range opts26 {
		...
		if bestSimilarity >= 0.8 && bestMatchIdx != -1 {
			...
			if ch25 != ch26 {
				if ch26 {
					status = "added" // checked in 2026 but not in 2025
				} else {
					status = "removed" // checked in 2025 but not in 2026
				}
			}
