...
var (
	// A real question number in this template is formatted as "4. Question text".
...
func main() {
	...
	for _, file := range files {
		...
		questions, err := parsePDF(resolvePDFPath(file))
		if err != nil {
...
func resolvePDFPath(file string) string {
	candidates := []string{file, "./" + file, "../" + file}
...
func parsePDF(path string) ([]Question, error) {
	...
	for pageNum := 1; pageNum <= numPages; pageNum++ {
		...
		// Parse questions from the left column. Questions can wrap across many
		// extracted text lines, so a pending question stays open until a structural
		...
		flushQuestion := func() {
			...
			qText := cleanJoinedText(strings.Join(currentQ.Parts, " "))
		...
		for _, el := range leftColumnText {
			...
			if matches, qNum, ok := detectQuestionStart(el, lastQuestionNumber); ok {
				flushQuestion()
				setLastQuestionEndY(el.Y)
				inComments = false
		...
		// Assign checkbox options, evidence, and attachments from the right column.
		// For a question at StartY, controls lie between StartY and EndY.
		for i := 0; i < len(pageParsedQuestions); i++ {
			parsed := &pageParsedQuestions[i]
			q := &parsed.Q
			...
			for _, re := range rightColumnText {
				// Use a small buffer above startY to capture aligned elements.
			...
			for _, qe := range qElements {
				...
				if isOptChecked || isOptUnchecked {
					...
					if strings.HasPrefix(cleanText, "Required evidence:") || strings.HasPrefix(cleanText, "Required Evidence:") {
						...
						currentEvidenceText = strings.TrimSpace(currentEvidenceText)
					} else {
						inOption = true
			...
			// Align respondent comments for this question.
			...
			if len(qComments) > 0 {
				joinedComments := cleanJoinedText(strings.Join(qComments, " "))
		...
		for _, parsed := range pageParsedQuestions {
			questions = append(questions, parsed.Q)
		}
	}

	for i := range questions {
		q := &questions[i]
		q.Text = cleanJoinedText(q.Text)
...
func detectQuestionStart(el TextElement, lastQuestionNumber int) ([]string, int, bool) {
	matches := qNumRegex.FindStringSubmatch(strings.TrimSpace(el.Text))
	if matches == nil {
		return nil, 0, false
	}

	qNum, err := strconv.Atoi(matches[1])
	if err != nil || qNum <= lastQuestionNumber {
		return nil, 0, false
	}

	// PortfolioView question numbers in this report are around x=46. Keeping a
	// narrow guard rejects respondent comments and score lines that are slightly
...
func cleanJoinedText(text string) string {
